...
Shumagin <- Shumagin %>%
  mutate(date = ymd(date),
         ...
         dummy_date = as.Date(paste0("2000-", hatch_DATE_noYear), format = "%Y-%b-%d")) %>%
...
kodiak.07.23 <- kodiak.07.23 %>%
  mutate(year = as.factor(year),
         bay = as.factor(bay),
         dummy_date = as.Date(paste0("2000-", kodiak.07.23$hatch_DATE_noYear), format = "%Y-%b-%d")) # this creates a "fake" date to help create figures
...
GAK.50.115.allyrs.hs <- GAK.50.115.allyrs %>%
  ...
  month.day = format(date, "%b-%d"),
  dummy_date = as.Date(paste0("2000-", month.day), format = "%Y-%b-%d"))
...
before.incub.temps.HS <- before.incub.temps %>%
  mutate(hatch.success = (1 / (1 + ((avg.22.temp - 4.192) / 2.125)^2)),
         dummy_date = as.Date(paste0("2000-", month.day), format = "%Y-%d-%b")) 
...
# The following sections create each hatch success plot for Eastern Kodiak and the Shumagin Islands
...
before.kodiak.hs.plot <- ggplot(GAK.50.115.before.hs, aes(x = dummy_date, y = hatch.success)) +
  ...
  geom_area(data = density.all.df.before, aes(x = x, y = y), fill = "black", position = "identity", inherit.aes = FALSE) +
  scale_x_date(
    date_labels = "%b-%d",
    date_breaks = "1 month",
  ...
  annotate("text", x = as.Date("2000-01-15"), y = 1, label = "Before", size = 10, fontface = "bold", hjust = 0)
...
# March 13 to May 6th - Falls within area of highest hatch success
...
adjacent.kodiak.hs.plot <- ggplot(GAK.50.115.adjacent.hs, aes(x = dummy_date, y = hatch.success)) +
  geom_smooth(color = "black", linewidth = 2, se = FALSE) +
  geom_area(data = density.all.df.adjacent, aes(x = x, y = y), fill = "black", position = "identity", inherit.aes = FALSE) +
  scale_x_date(
    date_labels = "%b-%d",
    date_breaks = "1 month",
    limits = as.Date(c("2000-01-15", "2000-06-15"))
  ...
  annotate("text", x = as.Date("2000-01-15"), y = 1, label = "Adjacent", size = 10, fontface = "bold", hjust = 0)
...
during.kodiak.hs.plot <- ggplot(GAK.50.115.during.hs, aes(x = dummy_date, y = hatch.success)) +
  geom_smooth(color = "black", linewidth = 2, se = FALSE) +
  geom_area(data = density.all.df.during, aes(x = x, y = y), fill = "black", position = "identity", inherit.aes = FALSE) +
  scale_x_date(
    date_labels = "%b-%d",
    date_breaks = "1 month",
    limits = as.Date(c("2000-01-15", "2000-06-15"))
  ...
  annotate("text", x = as.Date("2000-01-15"), y = 1, label = "During", size = 10, fontface = "bold", hjust = 0)
...
# Feb 27 to April 17 - Falls within area of highest hatch success
...
after.kodiak.hs.plot <- ggplot(GAK.50.115.after.hs, aes(x = dummy_date, y = hatch.success)) +
  geom_smooth(color = "black", linewidth = 2, se = FALSE) +
  geom_area(data = density.all.df.after, aes(x = x, y = y), fill = "black", position = "identity", inherit.aes = FALSE) +
  scale_x_date(
    date_labels = "%b-%d",
    date_breaks = "1 month",
    limits = as.Date(c("2000-01-15", "2000-06-15"))
  ...
  annotate("text", x = as.Date("2000-01-15"), y = 1, label = "After", size = 10, fontface = "bold", hjust = 0)
...
# Feb 17 to May 3rd - Falls within area of highest hatch success
...
all.fish.18.23.during <- all.fish.18.23 %>%
  ...
  mutate(dummy_date = as.Date(paste0("2000-", hatch_DATE_noYear), format = "%Y-%b-%d"),
         year = as.factor(year))
...
before.shum.hs.plot <- ggplot(Shumagin.hs.before, aes(x = dummy_date, y = hatch.success)) +
  ...
  scale_y_continuous(
    limits = c(0, 1), sec.axis = sec_axis(~ .)  # Blank secondary axis
  ) +
  labs(x = NULL, y = NULL) +
  theme_classic() +
  theme(
...
during.shum.hs.plot <- ggplot(Shumagin.hs.during, aes(x = dummy_date, y = hatch.success)) +
  geom_smooth(color = "black", linewidth = 2, se = FALSE) +
  geom_area(data = density.all.df.shum.during, aes(x = x, y = y), fill = "black", position = "identity", inherit.aes = FALSE) +
  scale_x_date(
    date_labels = "%b-%d",
    date_breaks = "1 month",
    limits = as.Date(c("2000-01-15", "2000-06-15"))
  ...
  scale_y_continuous(
    limits = c(0, 1), sec.axis = sec_axis(~ . / density.scale.shum.during)
  ) +
  labs(x = NULL, y = NULL) +
  theme_classic() +
...
# March 22 to May 16th - Falls within area of highest hatch success
...
adjacent.shum.hs.plot <- ggplot(Shumagin.hs.adjacent, aes(x = dummy_date, y = hatch.success)) +
  geom_smooth(color = "black", linewidth = 2, se = FALSE) +
  geom_area(data = density.all.df.shum.adjacent,aes(x = x, y = y),
            fill = "black", position = "identity", inherit.aes = FALSE) +
...
after.shum.hs.plot <- ggplot(Shumagin.hs.after, aes(x = dummy_date, y = hatch.success)) +
  geom_smooth(color = "black", linewidth = 2, se = FALSE) +
  geom_area(data = density.all.df.shum.after, aes(x = x, y = y), 
            fill = "black", position = "identity", inherit.aes = FALSE) +
  scale_x_date(date_labels = "%b-%d",
               date_breaks = "1 month",
               limits = as.Date(c("2000-01-15", "2000-06-15"))
...
# April 2 to May 15th - Falls within area of highest hatch success
...
# Formatting the density data

doy_all <- as.numeric(format(kodiak.07.23$dummy_date, "%j"))
density_all <- density(doy_all)
max_density_all <- max(density_all$y)

density_by_year_df <- kodiak.07.23 %>%
  mutate(doy = as.numeric(format(dummy_date, "%j"))) %>%
  group_by(year) %>%
  do({
    d <- density(.$doy)
  ...
  ungroup()
max_density_by_year <- max(density_by_year_df$y)
max_density <- max(max_density_all, max_density_by_year)

# 2. Scale all density curves so their max is, 1
density_scale <- 1 / max_density

density_all_df <- data.frame(
  x = as.Date(density_all$x, origin = "2000-01-01"),
  y = density_all$y * density_scale
)
density_by_year_df$y <- density_by_year_df$y * density_scale

density_by_year_df <- density_by_year_df %>%
  mutate(year_group = case_when(
    year %in% c(2007, 2009, 2010, 2012, 2013, 2014) ~ "Group1",
    year %in% c(2015, 2016, 2017, 2019) ~ "Group2",
    year %in% c(2018, 2020) ~ "Group3",
...
base.year.hs.plot <- ggplot(GAK.50.115.allyrs.hsplot, aes(dummy_date, hatch.success, color = year_group)) + 
  ...
  geom_area(data = density_by_year_df, aes(x = x, y = y, fill = year_group), position = "identity", inherit.aes = FALSE) +
  facet_wrap(~year) +
  scale_color_manual(values = c("Group1" = "darkblue",
                                ...
                                "Group4" = "steelblue1",
                                "Group5" = "black")) +
  scale_fill_manual(values = c("Group1" = "darkblue",
                               "Group2" = "darkred",
                               "Group3" = "lightpink",
                               "Group4" = "steelblue1",
                               "Group5" = "black")) +
...
all.fish.18.23.hsplot <- all.fish.18.23 %>%
...
doy_all_shum <- as.numeric(format(all.fish.18.23.hsplot$dummy_date, "%j"))
density_all_shum <- density(doy_all_shum)
max_density_all_shum <- max(density_all_shum$y)

density_by_year_df_shum <- all.fish.18.23.hsplot %>%
  mutate(doy = as.numeric(format(dummy_date, "%j"))) %>%
  group_by(year) %>%
  do({
    d <- density(.$doy)
  ...
  ungroup()
max_density_by_year_shum <- max(density_by_year_df_shum$y)
max_density_shum <- max(max_density_all_shum, max_density_by_year_shum)

# 2. Scale all density curves so their max is, 1
density_scale_shum <- 1 / max_density_shum

density_all_df_shum <- data.frame(
  x = as.Date(density_all_shum$x, origin = "2000-01-01"),
  y = density_all_shum$y * density_scale_shum
)
density_by_year_df_shum$y <- density_by_year_df_shum$y * density_scale_shum

density_by_year_df_shum <- density_by_year_df_shum %>%
  mutate(year_group = case_when(
    year %in% c(2007, 2009, 2010, 2012, 2013, 2014) ~ "Group1",
    year %in% c(2015, 2016, 2017, 2019) ~ "Group2",
    year %in% c(2018, 2020) ~ "Group3",
...
# Create the base plot
base.year.hs.plot.shum <- ggplot(Shumagin.hsplot, aes(dummy_date, hatch.success, color = year_group)) + 
  geom_line(linewidth = 2) +
  geom_area(data = density_by_year_df_shum, aes(x = x, y = y, fill = year_group), position = "identity", inherit.aes = FALSE) +
  facet_wrap(~year) +
  scale_color_manual(values = c("Group1" = "darkblue",
                                "Group2" = "darkred",
                                "Group3" = "lightpink",
  ...
  scale_fill_manual(values = c("Group1" = "darkblue",
                               ...
                               "Group5" = "black")) +
  xlab("Date") + 
  ylab("Shumagin Islands Hatch Success") +
  labs(color = NULL) +
  scale_x_date(
    breaks = as.Date(c("2000-02-01", "2000-04-01", "2000-06-01")), 
    date_labels = "%b-%d", 
  ...
  scale_y_continuous(
    limits = c(0, 1),
    sec.axis = sec_axis(~ . * max_density_shum)
  ) +
  theme_classic() +
  theme(axis.title = element_text(size = 32, face = "bold"),
        axis.text = element_text(size = 24),
...
# Convert to grob and modify strip colors
base.year.hs.plot.shum.final <- ggplotGrob(base.year.hs.plot.shum)

# Function to get color for a year
get_year_color <- function(year) {
  if(length(year) == 0 || is.na(year)) return("black")
  if(year %in% c(2007, 2009, 2010, 2012, 2013, 2014)) return("darkblue")
  if(year %in% c(2015, 2016, 2017, 2019)) return("darkred")
  if(year %in% c(2018, 2020)) return("lightpink")
...
# Find strip indices
strip_indices <- which(grepl('strip', base.year.hs.plot.shum.final$layout$name))

# Modify each strip
for(i in strip_indices) {
  tryCatch({
    # Navigate through the grob structure to find the text label
    strip_grob <- base.year.hs.plot.shum.final$grobs[[i]]
    ...
    if(!is.null(strip_text) && length(strip_text) > 0) {
      ...
      if(!is.na(year_value)) {
        # Set the color based on the year
        base.year.hs.plot.shum.final$grobs[[i]]$grobs[[1]]$children[[2]]$children[[1]]$gp$col <- get_year_color(year_value)
      }
    }
  }, error = function(e) {
    message(paste("Error processing strip", i, ":", e$message))
...
# Draw the plot
grid.newpage()
grid.draw(base.year.hs.plot.shum.final)
