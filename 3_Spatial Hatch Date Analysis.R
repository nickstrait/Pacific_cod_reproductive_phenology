...
HD.regions <- ggplot(all.fish.18.23, aes(region, as.Date(hatch_dummyDate, format = "%b-%d"), fill = region)) + 
  geom_violin(color = "gray") +
  xlab("Region") + 
  ylab("Hatch Date") +
  labs(fill = NULL) +
  scale_y_date(date_breaks = "1 month", date_labels = "%b-%d", limits = as.Date(c("2020-01-15", "2020-06-15"))) +
  scale_fill_manual(values = region.colors) +
  geom_hline(yintercept = as.Date("2020-04-09"),
             color = "black",
             linewidth = 0.75,
             linetype = "dashed") +
  theme_classic() +
...
combined.hdate <- (HD.regions / HD.bays) +
  plot_layout(ncol = 1, nrow = 2) & 
  theme(legend.position = "top",
        axis.title.y = element_blank())

combined.hdate <- grid.arrange(
  patchworkGrob(combined.hdate),
  left = textGrob("Hatch Date", rot = 90, gp = gpar(fontsize = 36, fontface = "bold"))
)
...
mmwestgulf.df <- mmwestgulf.df %>%
  mutate(dummy.hdate = as.Date(round(mmwestgulf.df$emmean), origin = "2000-01-01"),
         dummy.lwr.Cl = as.Date(round(mmwestgulf.df$lower.CL), origin = "2000-01-01"),
         dummy.upr.Cl = as.Date(round(mmwestgulf.df$upper.CL), origin = "2000-01-01"),
         dataset = "West Gulf")



eastern.kodiak.07.23 <- eastern.kodiak.07.23 %>%
  mutate(dummy.hdate = as.Date(round(eastern.kodiak.07.23$emmean), origin = "2000-01-01"),
         dummy.lwr.Cl = as.Date(round(eastern.kodiak.07.23$lower.CL), origin = "2000-01-01"),
         dummy.upr.Cl = as.Date(round(eastern.kodiak.07.23$upper.CL), origin = "2000-01-01"),
...
ggplot(combined.mm, aes(bay.dataset, dummy.hdate, color = HW)) +
  ...
  scale_y_date(date_breaks = "1 month", date_labels = "%b-%d", limits = as.Date(c("2000-02-15", "2000-05-15"))) +
